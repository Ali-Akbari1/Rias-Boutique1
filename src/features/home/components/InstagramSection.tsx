...
import { getInstagramCards, getInstagramProfileUrl } from "@/features/store/data/store-content";
    ...
    <section id="instagram" className="bg-background py-16 sm:py-20">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="rounded-xl border border-border bg-card/30 p-5 sm:p-8">
          <Reveal className="mb-8 flex flex-col gap-5 border-b border-border pb-6 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              <Button asChild className="h-11 px-6 text-base font-semibold group">
                <a href={profileUrl} target="_blank" rel="noreferrer">
                  <Instagram className="h-4 w-4 transition-transform duration-200 ease-out group-hover:-translate-y-0.5" />
                  View Profile
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {cards.map((card, index) => (
                <Reveal key={card.id} delayMs={index * 90}>
                  <article className="group overflow-hidden rounded-lg border border-border bg-background shadow-boutique transition-transform duration-300 hover:-translate-y-1 hover:shadow-card-hover">
                    ...
                    <div className="space-y-3 border-t border-border bg-background px-4 py-3">
                      ...
                      >
                        Open on Instagram
                        <ExternalLink className="h-3.5 w-3.5 transition-transform duration-200 ease-out group-hover:-translate-y-0.5" />
                      </a>
                    </div>
                  </article>
                </Reveal>
